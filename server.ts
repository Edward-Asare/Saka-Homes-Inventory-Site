...
async function startServer() {
      ...
      contentSecurityPolicy: isProduction
        ? {
            ...
            directives: {
              ...
              connectSrc: ["'self'", "https://*.supabase.co", "wss://*.supabase.co", "https://wa.me", supabaseOrigin].filter(Boolean) as string[],
              objectSrc: ["'none'"],
              baseUri: ["'self'"],
              formAction: ["'self'"],
              frameAncestors: ["'none'"],
  ...
  // Health check is registered before rate limiters so probes are not throttled.
